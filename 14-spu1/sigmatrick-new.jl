...
# SPSU1 symmetry type (2x2 matrix case)
...
function readspecs(imp_ops, self_ops, path, spin; prefix="spec_FDM_dens", suffix=".dat")
    names = Dict()
    names["G" * "-" * spin] = makespecname(imp_ops, imp_ops)
    names["Fl" * "-" * spin] = makespecname(self_ops, imp_ops)
    names["Fr" * "-" * spin] = makespecname(imp_ops, self_ops)
    names["I" * "-" * spin] = makespecname(self_ops, self_ops)
    
    n = length(imp_ops) * length(imp_ops)

    specdata = Dict()

    for matrix in ["G" * "-" * spin, "Fl" * "-" * spin, "Fr" * "-" * spin, "I" * "-" * spin]
        corr = ComplexF64[]
        for i in 1:n
            name = names[matrix][i]
            filenameim = prefix * "_" * name * "-" * spin * suffix
            dataim = readdlm(joinpath(path, filenameim))
            filenamere = prefix * "_" * name * "-" * spin * "-re" * suffix
            datare = readdlm(joinpath(path, filenamere))
            # be careful with the -pi prefactor
            col = -pi*(datare[:, 2] .+ im*dataim[:, 2])
            # col = (datare[:, 2] .+ im*dataim[:, 2])
            if i == 1
                corr = [dataim[:, 1] col]
            else
                corr = hcat(corr, col)
...
Δ = readhybri()
for spin in ["u", "d"]
    G, Fl, Fr, I = readspecs(imp_ops, self_ops, path, spin)
    ...
    sigmaHm = zeros(2, 2)
    if spin == "u"
        sigmaH11 = parse(Float64, readchomp(`extractcolumn custom.avg SigmaHd11-u`))
        sigmaH12 = parse(Float64, readchomp(`extractcolumn custom.avg SigmaHd12-u`))
        sigmaH21 = parse(Float64, readchomp(`extractcolumn custom.avg SigmaHd21-u`))
        sigmaH22 = parse(Float64, readchomp(`extractcolumn custom.avg SigmaHd22-u`))
        # it should be like this, which I manually set
        sigmaHm = [sigmaH11 sigmaH12; sigmaH21 sigmaH22];
    elseif spin == "d"
        sigmaH11 = parse(Float64, readchomp(`extractcolumn custom.avg SigmaHd11-d`))
        sigmaH12 = parse(Float64, readchomp(`extractcolumn custom.avg SigmaHd12-d`))
        sigmaH21 = parse(Float64, readchomp(`extractcolumn custom.avg SigmaHd21-d`))
        sigmaH22 = parse(Float64, readchomp(`extractcolumn custom.avg SigmaHd22-d`))
        # it should be like this, which I manually set
        sigmaHm = [sigmaH11 sigmaH12; sigmaH21 sigmaH22];
    end
